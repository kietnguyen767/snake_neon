...
export class GameScene extends Phaser.Scene {
  ...
  update(_time: number, _delta: number) {
    ...
    if (this.room?.state?.players) {
      this.room.state.players.forEach((player: ColyseusPlayer, sessionId: string) => {
        if (!this.playersHead.has(sessionId)) {
          this.addPlayer(player, sessionId);
        }
      });

      this.playersHead.forEach((_headRect, sessionId) => {
    ...
    if (this.room?.state?.foods) {
      this.room.state.foods.forEach((food: ColyseusFood, foodId: string) => {
        if (!this.foods.has(foodId)) {
          this.addFood(food, foodId);
        }
      });

      this.foods.forEach((_foodObject, foodId) => {
        if (!this.room?.state?.foods?.has(foodId)) {
    ...
    if (this.listenersAttached) {
      this.playersHead.forEach((headRect, sessionId) => {
        ...
        if (isLocalPlayer) {
          ...
          const dx = htx - headRect.x;
          const dy = hty - headRect.y;
          const snapDistanceSq = (this.tileSize * 1.5) * (this.tileSize * 1.5);
  ...
  private syncPlayerBody(
    ...
    while (bodyArr.length < segmentCount) {
  ...
  private addPlayer(player: ColyseusPlayer, sessionId: string) {
    ...
    if (isMe) {
      this.cameras.main.startFollow(headRect, true, 0.22, 0.22);
    }
  ...
  private addFood(food: ColyseusFood, foodId: string) {
    ...
    let color = 0xffffff;
    if (food.type === 1) color = 0x3cd7ff;
    else if (food.type === 2) color = 0xffdb40;
    else if (food.type === 3) color = 0xd84eff;
  ...
  private removeFood(foodId: string) {
    const foodObject = this.foods.get(foodId);
    if (foodObject) {
      foodObject.destroy();
    }
  ...
  attachListeners() {
    ...
    if (room.state.foods) {
      room.state.foods.forEach((food: ColyseusFood, foodId: string) => {
        this.addFood(food, foodId);
      });
      room.state.foods.onAdd((food: ColyseusFood, foodId: string) => {
        this.addFood(food, foodId);
      });
